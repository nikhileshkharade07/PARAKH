...
def search(
    ...
    for c in contract_matches:
        crs_val = c.risk_assessment.crs if hasattr(c, 'risk_assessment') and c.risk_assessment else (c.risk_score * 100 if c.risk_score else 45)
    ...
    vendor_matches = (
        ...
        .filter(
            or_(
                ...
                Vendor.pan.ilike(term) if hasattr(Vendor, 'pan') else False,
                Vendor.cin.ilike(term) if hasattr(Vendor, 'cin') else False
            )
        )
        .limit(limit)
        .all()
    ...
    dept_matches = (
        ...
        .filter(
            or_(
                ...
                Department.code.ilike(term) if hasattr(Department, 'code') else False
            )
        )
        .limit(limit)
        .all()
    ...
    case_matches = (
        ...
        .filter(
            or_(
                ...
                InvestigationCase.notes.ilike(term)
            )
        )
        .limit(limit)
        .all()
