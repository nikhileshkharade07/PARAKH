"""
Seed script for generating a small, deterministic test dataset for PARAKH.
This dataset includes both normal and suspicious records to test the risk pipeline.
"""
import sys
...
def main():
    ...
    c1 = Contract(
        ...
        contract_date=datetime(2024, 1, 15).date(),
    ...
    c2 = Contract(
        ...
        contract_date=datetime(2024, 2, 1).date(),
        department_id=dept_public_works.id,
        vendor_id=vendor_apex.id,
    ...
    c3 = Contract(
        ...
        contract_date=datetime(2024, 3, 1).date(),
        department_id=dept_public_works.id,
    ...
    # Contract 4: Vendor lock-in (RF-2) - we'll make this vendor win 3 out of 4 contracts in the department
    # We already have c1 and c2 and c3 in Public Works. Let's make this the fourth contract and make the same vendor win.
    # But note: we need to have the vendor win >60% of contracts. We'll have 3 contracts total in the department for this test.
    # Let's adjust: we'll create two more contracts in Public Works (c4 and c5) and make the same vendor win all three.
    # Then we'll have 3 contracts, all won by the same vendor -> 100% > 60% -> RF-2.
    # However, we already have c1, c2, c3. We'll change c1 and c2 to be won by a different vendor?
    # Let's redesign: we'll create three contracts in Public Works for vendor lock-in test.
    # We'll use c4, c5, c6 for this.
    # But we already used c1, c2, c3. Let's reassign:
    #   c1: normal -> we'll change it to be won by a different vendor to avoid affecting the lock-in test.
    #   c2: single bidder -> we'll change it to be won by a different vendor.
    #   c3: high value -> we'll change it to be won by a different vendor.
    # Then we'll create c4, c5, c6 for the lock-in test, all won by the same vendor.
    #
    # Let's restart the contract creation with a clear plan.
    #
    # We'll delete the above contracts and start over.
    #
    # Instead of rewriting, let's clear the contracts we added and start fresh.
    #
    # For simplicity, let's start over in this function.
    #
    # We'll delete the contracts we just added and then create a new set.
    #
    # But note: we are in the middle of the function and we have already added c1, c2, c3.
    # Let's rollback and start over.
    #
    # We'll do a rollback and then create the contracts again.
    #
    db.rollback()
    #
    # Now, let's create the departments and vendors again (since we rolled back).
    #
    # We'll keep the same department and vendor objects.
    #
    # Actually, we rolled back the entire transaction, so we need to recreate the departments and vendors.
    #
    # Let's recreate the departments and vendors.
    #
    dept_public_works = Department(name="Public Works Department")
    dept_health = Department(name="Health Services Directorate")
    dept_education = Department(name="Education Infrastructure Division")
    ...
    vendor_edubuild = Vendor(name="EduBuild Projects",
                             product_description="school furniture classroom equipment education infrastructure")
    db.add_all([dept_public_works, dept_health, dept_education, dept_digital,
                vendor_apex, vendor_bharat, vendor_civictech, vendor_medsupply, vendor_edubuild])
    db.flush()
    #
    # Now, let's create the contracts again with a clear plan.
    #
    # We'll create:
    #   c1: Normal contract (low risk) - in Public Works, vendor Apex, 3 bidders
    #   c2: Single bidder (RF-1) - in Public Works, vendor Apex, 1 bidder
    #   c3: High award value (RF-5) - in Public Works, vendor Bharat, 40% over estimate
    #   c4, c5, c6: Three contracts in Public Works, all won by vendor Apex -> vendor lock-in (RF-2) for vendor Apex in Public Works
    #   c7: Compressed tender window (RF-4) - in Health, vendor MedSupply, 3-day window
    #   c8: Specification tailoring (RF-7) - in Education, vendor CivicTech, specification matches vendor description
    #   c9: Unusual extensions (RF-8) - in Digital, vendor EduBuild, two extensions of 100+ days each
    #   c10: Multiple flags (RF-1, RF-5, RF-7) - in Public Works, vendor Apex, single bidder, high value, and tailored spec
    #
    # Let's create them.
    #
    # Contract 1: Normal contract (low risk)
    ...
    c4 = Contract(
        ...
        contract_date=datetime(2024, 4, 1).date(),
        department_id=dept_public_works.id,
        vendor_id=vendor_apex.id,
    ...
    c5 = Contract(
        ...
        contract_date=datetime(2024, 5, 1).date(),
        department_id=dept_public_works.id,
        vendor_id=vendor_apex.id,
    ...
    c6 = Contract(
        ...
        contract_date=datetime(2024, 6, 1).date(),
        department_id=dept_public_works.id,
        vendor_id=vendor_apex.id,
    ...
    contracts.append(c6)
    # Now, in Public Works department, we have 6 contracts (c1, c2, c3, c4, c5, c6).
    # Vendor Apex won: c1, c2, c4, c5, c6 -> 5 out of 6 -> 83% > 60% -> triggers RF-2 for vendor Apex in Public Works.
    ...
    c7 = Contract(
        ...
        contract_date=datetime(2024, 4, 15).date(),
    ...
    c8 = Contract(
        ...
        contract_date=datetime(2024, 5, 15).date(),
    ...
    c10 = Contract(
        ...
        contract_date=datetime(2024, 6, 15).date(),
        department_id=dept_education.id,
        vendor_id=vendor_civictech.id,
    ...
    # Now, we have created all contracts. We need to commit the session so that the contracts are persisted.
    db.commit()

    # Now, we run the risk engine on each contract to compute the risk assessments.
    # We'll create a risk engine and analyze each contract.
    risk_engine = RiskEngine()
    for contract in contracts:
        # We need to refresh the contract from the database to get the related objects (like department and vendor) because we are in the same session.
        # But note: we are in the same session and we just committed, so the objects are already in the session.
        # However, to be safe, we can query the contract again.
        # But we are in the same session and we have the contract object, so we can use it.
        risk_engine.analyze_contract(contract, db)

    # Commit again to save the risk assessments.
    db.commit()
